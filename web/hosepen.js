...
function showContextMenu(x, y) {
    ...
    const menuItems = [
        ...
        { text: '🧑 打开火柴人编辑器', action: () => openStickmanEditor() }
    ];
...
// 打开火柴人编辑器
function openStickmanEditor() {
    // 检查是否已存在火柴人编辑器窗口
    let stickmanWindow = document.querySelector('.stickman-editor-window');
    if (stickmanWindow) {
        // 检查窗口内容是否加载成功（是否有iframe）
        const hasIframe = stickmanWindow.querySelector('iframe');
        if (!hasIframe) {
            // 如果窗口存在但没有iframe，说明之前加载失败，删除旧窗口重新创建
            console.log('[hosepen] 检测到窗口存在但内容加载失败，重新创建窗口');
            if (stickmanWindow._cleanup) {
                stickmanWindow._cleanup();
            }
            stickmanWindow.remove();
            stickmanWindow = null;
        } else {
            // 如果已存在且内容正常，显示窗口
            stickmanWindow.style.display = 'flex';
            console.log('[hosepen] 显示已存在的火柴人编辑器窗口');
            
            // 确保消息监听器仍然存在（防止被意外移除）
            const iframe = stickmanWindow.querySelector('iframe');
            if (iframe && !stickmanWindow._messageHandler) {
                console.log('[hosepen] 检测到消息监听器丢失，重新创建...');
                // 重新创建消息监听器
                const messageHandler = (event) => {
                    try {
                        ...
                        // 检查消息类型和格式
                        if (!data || typeof data !== 'object' || data.type !== 'exportImage') {
                            return;
                        }
                        
                        if (!data.imageDataUrl) {
                            console.warn('[hosepen] 收到格式不正确的导出图像消息');
                            return;
                        }
                        
                        console.log('[hosepen] 收到火柴人编辑器导出图像请求，数据大小:', data.imageDataUrl.length);
                        createStickmanImageOutputNode(data.imageDataUrl);
                    } catch (error) {
                        console.error('[hosepen] 处理火柴人编辑器消息失败:', error);
                        alert('处理导出图像请求失败: ' + error.message);
                    }
                };
                
                window.addEventListener('message', messageHandler);
                stickmanWindow._messageHandler = messageHandler;
                stickmanWindow._iframe = iframe;
            }
    ...
    // 创建火柴人编辑器窗口
    stickmanWindow = document.createElement('div');
    stickmanWindow.className = 'stickman-editor-window';
    stickmanWindow.style.cssText = `
        position: fixed;
        top: 50%;
        left: 50%;
        transform: translate(-50%, -50%);
    ...
    closeBtn.addEventListener('click', () => {
        stickmanWindow.style.display = 'none';
        // 清理资源
        if (stickmanWindow._cleanup) {
            stickmanWindow._cleanup();
        }
    ...
    function getBasePath() {
        ...
        for (let script of scripts) {
            if (script.src && script.src.includes('hosepen.js')) {
                const scriptPath = script.src;
                console.log('[hosepen] 检测到 hosepen.js 路径:', scriptPath);
                // 提取基础路径
                const match = scriptPath.match(/^(.*\/)hosepen\.js/);
                if (match) {
                    const basePath = match[1];
                    console.log('[hosepen] 推断的基础路径:', basePath);
                    return basePath;
                }
            }
        }
    ...
    // 尝试多个可能的路径（ComfyUI web文件路径格式）
    const basePath = getBasePath();
    const possiblePaths = basePath ? [
        basePath + 'stickman-editor.html',                  // 基于当前脚本路径
        basePath.replace(/\/web\//, '/').replace(/\/hosepen\.js$/, '') + '/stickman-editor.html'
    ] : [];
    
    // 添加其他可能的路径（优先使用自定义路由）
    possiblePaths.unshift('/hosepen/stickman-editor.html');  // 自定义路由（最优先）
    possiblePaths.push(
        '/customnodes/comfy_hosepen/stickman-editor.html',  // ComfyUI标准路径
        '/web/comfy_hosepen/stickman-editor.html',          // 备用路径
        './stickman-editor.html',                           // 相对路径
        '/web/stickman-editor.html',                        // 绝对web路径
        'stickman-editor.html'                              // 简单文件名
    );
    ...
    async function tryLoadFile() {
        ...
        for (const path of possiblePaths) {
            try {
                console.log(`[hosepen] 尝试加载路径: ${path}`);
                const response = await fetch(path);
                console.log(`[hosepen] 响应状态: ${response.status} ${response.statusText}`);
                if (response.ok) {
                    const text = await response.text();
                    console.log(`[hosepen] ✓ 成功从路径加载: ${path} (大小: ${text.length} 字符)`);
                    return text;
                } else {
                    errors.push(`${path}: ${response.status} ${response.statusText}`);
                    console.log(`[hosepen] ✗ 路径 ${path} 返回状态: ${response.status}`);
                }
            } catch (error) {
                errors.push(`${path}: ${error.message}`);
                console.log(`[hosepen] ✗ 尝试路径 ${path} 失败:`, error);
                continue;
            }
        }
        const errorMsg = '无法从任何路径加载HTML文件。尝试的路径:\n' + errors.join('\n');
        console.error('[hosepen]', errorMsg);
        throw new Error(errorMsg);
    }
    
    tryLoadFile()
        .then(htmlContent => {
            // 创建iframe并设置内容为data URL
            const iframe = document.createElement('iframe');
            ...
            const messageHandler = (event) => {
                try {
                    ...
                    console.error('[hosepen] 处理火柴人编辑器消息失败:', error);
                    console.error('[hosepen] 错误堆栈:', error.stack);
                    alert('处理导出图像请求失败: ' + error.message);
                }
            };
            
            // 如果之前有消息监听器，先移除
            if (stickmanWindow._messageHandler) {
                window.removeEventListener('message', stickmanWindow._messageHandler);
            }
            
            window.addEventListener('message', messageHandler);
            
            // 将iframe的引用和消息处理器存储到窗口对象，以便清理时移除监听器
            stickmanWindow._messageHandler = messageHandler;
            stickmanWindow._iframe = iframe;
        ...
        .catch(error => {
            console.error('[hosepen] 加载HTML文件失败:', error);
            // 显示错误信息
            ...
            `;
            errorMsg.innerHTML = `
                <p style="font-size: 16px; color: #d32f2f; margin-bottom: 10px;">无法加载火柴人编辑器</p>
                <p style="font-size: 12px; color: #999; margin-bottom: 15px;">请打开浏览器控制台（F12）查看详细错误信息</p>
                <p style="font-size: 11px; color: #666; text-align: left; background: #f5f5f5; padding: 10px; border-radius: 4px; max-height: 200px; overflow-y: auto;">
    ...
    stickmanWindow.appendChild(header);
    stickmanWindow.appendChild(contentContainer);
    ...
    header.addEventListener('mousedown', (e) => {
        ...
        const rect = stickmanWindow.getBoundingClientRect();
        windowStartX = rect.left;
        windowStartY = rect.top;
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        stickmanWindow.style.left = newX + 'px';
        stickmanWindow.style.top = newY + 'px';
        stickmanWindow.style.transform = 'none';
    });
    ...
    const escKeyHandler = (event) => {
        if (event.key === 'Escape') {
            const stickmanVisible = stickmanWindow && 
                                  stickmanWindow.style.display === 'flex' && 
                                  document.body.contains(stickmanWindow);
            
            if (stickmanVisible) {
                stickmanWindow.style.display = 'none';
                console.log('[hosepen] ESC键关闭火柴人编辑器窗口');
            }
        }
    };
    ...
    // 存储清理函数，当窗口被移除时清理监听器
    stickmanWindow._escKeyHandler = escKeyHandler;
    
    document.body.appendChild(stickmanWindow);
    console.log('[hosepen] 创建火柴人编辑器窗口 (支持ESC关闭)');
}
...
// 记录最后创建的火柴人编辑器节点
let lastStickmanNode = null;

// 上传图像到ComfyUI（火柴人编辑器专用，上传到stickman子文件夹）
async function uploadStickmanImageToComfyUI(base64Data, fileName) {
    try {
        // 将base64转换为Blob
        const byteCharacters = atob(base64Data);
        const byteNumbers = new Array(byteCharacters.length);
        ...
        formData.append('image', blob, fileName);
        formData.append('type', 'input');
        formData.append('subfolder', 'stickman');  // 上传到stickman子文件夹
        ...
        const result = await response.json();
        console.log('[hosepen] 火柴人图像上传成功:', result);
        return result;
        
    } catch (error) {
        console.error('[hosepen] 上传火柴人图像失败:', error);
        throw error;
    }
}

// 创建或更新火柴人编辑器 Image Output节点
function createStickmanImageOutputNode(imageDataUrl) {
    try {
        console.log('[hosepen] 创建或更新火柴人编辑器 Image Output节点...');
        
        // 检查是否在ComfyUI环境中
        if (typeof app === 'undefined' || !app.graph || typeof LiteGraph === 'undefined') {
            console.error('[hosepen] 未检测到ComfyUI环境');
            alert('请在ComfyUI中使用此功能');
            return Promise.reject(new Error('未检测到ComfyUI环境'));
        }
        
        // 验证图像数据
        if (!imageDataUrl || typeof imageDataUrl !== 'string') {
            console.error('[hosepen] 无效的图像数据');
            alert('无效的图像数据');
            return Promise.reject(new Error('无效的图像数据'));
        }
        
        // 从data URL中提取base64数据
        const parts = imageDataUrl.split(',');
        if (parts.length < 2) {
            console.error('[hosepen] 图像数据格式错误');
            alert('图像数据格式错误');
            return Promise.reject(new Error('图像数据格式错误'));
        }
        
        const base64Data = parts[1];
        
        // 检查是否已有节点存在，如果存在则使用固定文件名（覆盖），否则使用时间戳文件名
        const existingNodes = app.graph._nodes.filter(node => 
            node.type === 'StickmanImageOutput'
        );
        
        console.log('[hosepen] 检查节点状态，找到', existingNodes.length, '个火柴人编辑器节点');
        if (existingNodes.length > 0) {
            console.log('[hosepen] 现有节点信息:', {
                nodeId: existingNodes[0].id,
                currentFileName: existingNodes[0].widgets?.[0]?.value,
                nodeType: existingNodes[0].type
            });
        }
        
        let fileName;
        const shouldUpdateExisting = existingNodes.length > 0;
        if (shouldUpdateExisting) {
            // 如果节点已存在，使用固定文件名，这样IS_CHANGED能检测到文件修改时间变化
            fileName = 'stickman_export.png';
            console.log('[hosepen] 检测到现有节点，使用固定文件名:', fileName, '将覆盖现有文件');
        } else {
            // 如果节点不存在，使用时间戳文件名创建新节点
            const timestamp = Date.now();
            fileName = `stickman_export_${timestamp}.png`;
            console.log('[hosepen] 创建新节点，使用时间戳文件名:', fileName);
        }
        
        console.log('[hosepen] 开始上传火柴人图像到服务器...', { fileName, dataSize: base64Data.length });
        
        // 上传图像到ComfyUI服务器
        return uploadStickmanImageToComfyUI(base64Data, fileName)
            .then((result) => {
                console.log('[hosepen] 火柴人图像上传成功:', result);
                
                // 使用返回的文件名（服务器可能会修改文件名）
                const uploadedFileName = result.name || fileName;
                console.log('[hosepen] 上传后的文件名:', uploadedFileName, '原始文件名:', fileName, '应该更新现有节点:', shouldUpdateExisting);
                
                // 创建火柴人编辑器 Image Output节点
                // 传递原始的文件名决策信息，确保节点更新逻辑一致
                return createStickmanOutputNodeInGraph(uploadedFileName, shouldUpdateExisting);
            })
            .then((isNewNode) => {
                // isNewNode 为 true 表示创建了新节点，false 表示更新了现有节点
                // 提示消息已经在 createStickmanOutputNodeInGraph 中显示，这里不需要重复显示
                if (isNewNode) {
                    console.log('[hosepen] 火柴人编辑器 Image Output节点已添加');
                } else {
                    console.log('[hosepen] 火柴人编辑器 Image Output节点已更新');
                }
            })
            .catch((error) => {
                console.error('[hosepen] 创建节点失败:', error);
                console.error('[hosepen] 错误堆栈:', error.stack);
                alert('创建节点失败: ' + (error.message || String(error)));
                throw error;
            });
...
// 在图中创建火柴人编辑器 Image Output节点
async function createStickmanOutputNodeInGraph(fileName, shouldUpdateExisting = null) {
    try {
        // 查找所有火柴人编辑器导出的节点（不依赖lastStickmanNode，因为节点可能被删除）
        const existingNodes = app.graph._nodes.filter(node => 
            node.type === 'StickmanImageOutput'
        );
        
        // 如果shouldUpdateExisting为null，则根据existingNodes判断
        // 如果shouldUpdateExisting已指定，则使用该值（确保与文件名决策一致）
        const hasExistingNode = shouldUpdateExisting !== null ? shouldUpdateExisting : existingNodes.length > 0;
        
        console.log('[hosepen] 节点更新决策:', {
        ...
        // 如果找到现有节点，更新第一个
        if (hasExistingNode && existingNodes.length > 0) {
            const nodeToUpdate = existingNodes[0];
            const currentFileName = nodeToUpdate.widgets?.[0]?.value;
            console.log('[hosepen] 找到已存在的火柴人编辑器节点，更新图像...', {
                nodeId: nodeToUpdate.id,
                currentFileName,
                newFileName: fileName,
                fileNameChanged: currentFileName !== fileName
            });
            
            // 更新lastStickmanNode引用
            lastStickmanNode = nodeToUpdate;
            
            // 更新现有节点的图像文件名
            if (nodeToUpdate.widgets && nodeToUpdate.widgets[0]) {
                const oldValue = nodeToUpdate.widgets[0].value;
                
                // 如果文件名相同（使用固定文件名的情况），也需要强制触发更新
                const fileNameChanged = oldValue !== fileName;
                
                // 清除节点的缓存（如果存在）- 必须在更新值之前清除
                if (nodeToUpdate.last_loaded !== undefined) {
                    nodeToUpdate.last_loaded = null;
                }
                
                // 清除节点的图像缓存（如果存在）
                if (nodeToUpdate.image !== undefined) {
                    nodeToUpdate.image = null;
                }
                
                // 更新widget值（即使文件名相同也要更新，确保触发重新加载）
                nodeToUpdate.widgets[0].value = fileName;
                console.log('[hosepen] 更新节点文件名:', oldValue, '->', fileName, fileNameChanged ? '(文件名已改变)' : '(文件名相同，强制更新)');
                
                // 如果节点有配置对象，也更新它
                if (nodeToUpdate.properties) {
                    nodeToUpdate.properties.image = fileName;
                }
                
                // 标记节点为已更改（必须在更新值之后）
                if (typeof nodeToUpdate.setDirty === 'function') {
                    nodeToUpdate.setDirty(true);
                } else if (nodeToUpdate.dirty !== undefined) {
                    nodeToUpdate.dirty = true;
                }
                
                // 触发widget变化事件（如果存在）- 这应该会触发节点重新加载图像
                if (nodeToUpdate.widgets[0].callback) {
                    try {
                        nodeToUpdate.widgets[0].callback(fileName);
                    } catch (e) {
                        console.warn('[hosepen] widget callback 执行失败:', e);
                    }
                }
                
                // 触发节点配置更新（如果存在）
                if (typeof nodeToUpdate.onConfigure === 'function') {
                    try {
                        nodeToUpdate.onConfigure({ image: fileName });
                    } catch (e) {
                        console.warn('[hosepen] onConfigure 执行失败:', e);
                    }
                }
                
                // 触发节点更新事件（如果存在）
                if (typeof nodeToUpdate.onNodeUpdated === 'function') {
                    try {
                        nodeToUpdate.onNodeUpdated();
                    } catch (e) {
                        console.warn('[hosepen] onNodeUpdated 执行失败:', e);
                    }
                }
                
                // 如果节点有onWidgetChange方法，调用它
                if (typeof nodeToUpdate.onWidgetChange === 'function') {
                    try {
                        nodeToUpdate.onWidgetChange(nodeToUpdate.widgets[0], oldValue, fileName);
                    } catch (e) {
                        console.warn('[hosepen] onWidgetChange 执行失败:', e);
                    }
                }
                
                // 强制触发ComfyUI的节点重新验证
                // 通过修改节点的配置来触发重新验证
                if (nodeToUpdate.comfyuiNode) {
                    try {
                        // 如果节点有comfyuiNode属性，尝试触发更新
                        if (typeof nodeToUpdate.comfyuiNode.onNodeUpdated === 'function') {
                            nodeToUpdate.comfyuiNode.onNodeUpdated();
                        }
                    } catch (e) {
                        console.warn('[hosepen] comfyuiNode更新失败:', e);
                    }
                }
                
                // 如果文件名相同（使用固定文件名），强制触发节点重新验证
                // 通过临时改变widget值再改回来，强制触发更新
                if (!fileNameChanged) {
                    console.log('[hosepen] 文件名相同，强制触发节点重新验证');
                    try {
                        // 先清除所有缓存
                        if (nodeToUpdate.last_loaded !== undefined) {
                            nodeToUpdate.last_loaded = null;
                        ...
                        // 临时改变widget值，强制触发更新
                        const originalValue = nodeToUpdate.widgets[0].value;
                        nodeToUpdate.widgets[0].value = fileName + '_temp_' + Date.now();
                        
                        // 立即触发callback
                        if (nodeToUpdate.widgets[0].callback) {
                            try {
                                nodeToUpdate.widgets[0].callback(nodeToUpdate.widgets[0].value);
                            } catch (e) {
                                console.warn('[hosepen] 临时callback执行失败:', e);
                            }
                        }
                        
                        // 延迟改回原值，再次触发更新
                        setTimeout(() => {
                            if (app.graph._nodes.includes(nodeToUpdate)) {
                                nodeToUpdate.widgets[0].value = fileName;
                                console.log('[hosepen] 恢复文件名并强制触发更新:', fileName);
                                ...
                                if (nodeToUpdate.widgets[0].callback) {
                                    try {
                                        ...
                                        console.warn('[hosepen] 恢复后callback执行失败:', e);
                                    }
                                }
                        ...
                        console.warn('[hosepen] 强制更新失败:', e);
                    }
                }
                
                // 尝试通过修改节点标题来触发更新（这是一个hack，但可能有效）
                const originalTitle = nodeToUpdate.title;
                try {
                    nodeToUpdate.title = originalTitle + ' ';
                    setTimeout(() => {
                        if (app.graph._nodes.includes(nodeToUpdate)) {
                            nodeToUpdate.title = originalTitle;
                        }
            ...
            // 延迟刷新，确保文件已写入磁盘，并强制节点重新加载
            // 使用较长的延迟，确保文件系统操作完成
            setTimeout(() => {
                // 再次检查节点是否还在图中
                if (!app.graph._nodes.includes(nodeToUpdate)) {
                    console.warn('[hosepen] 节点在延迟刷新时已被删除，跳过');
                    return;
                }
                ...
                // 再次触发widget callback，确保节点重新加载
                if (nodeToUpdate.widgets && nodeToUpdate.widgets[0] && nodeToUpdate.widgets[0].callback) {
                    try {
                        nodeToUpdate.widgets[0].callback(nodeToUpdate.widgets[0].value);
                    } catch (e) {
                        console.warn('[hosepen] 延迟widget callback 执行失败:', e);
                    }
                }
                
                // 刷新画布（只刷新显示，不触发执行，避免execution_start警告）
                app.graph.setDirtyCanvas(true, false);
                
                // 如果节点有compute方法，尝试重新计算
                if (typeof nodeToUpdate.compute === 'function') {
                    try {
                        // 不实际执行，只是标记需要重新计算
                        nodeToUpdate.setDirty(true);
                    } catch (e) {
                        console.warn('[hosepen] 标记节点重新计算失败:', e);
                    }
                }
                
                console.log('[hosepen] 延迟刷新完成，节点应已检测到文件变化，文件名:', fileName);
            }, 800); // 增加延迟时间，确保文件已完全写入并避免时序问题
            
            // 额外延迟刷新，确保ComfyUI检测到文件变化
            setTimeout(() => {
                if (!app.graph._nodes.includes(nodeToUpdate)) {
                    return;
                }
                ...
                // 强制刷新画布（只刷新显示，不触发执行）
                app.graph.setDirtyCanvas(true, false);
                console.log('[hosepen] 第二次延迟刷新完成');
            }, 1000);
            
            // 显示更新提示消息
            showSuccessMessage('✅ 已更新火柴人编辑器导出图像节点（请重新执行工作流查看新图像）');
            console.log('[hosepen] 火柴人编辑器 Image Output节点已更新，文件名:', fileName);
            
            // 返回 false 表示更新了现有节点
            return false;
        }
        
        // 创建新的火柴人编辑器 Image Output节点
        console.log('[hosepen] 创建新的火柴人编辑器 Image Output节点...');
        
        const node = LiteGraph.createNode("StickmanImageOutput");
        
        if (!node) {
            console.error('[hosepen] 无法创建StickmanImageOutput节点');
            alert('火柴人编辑器导出图像节点未找到，请重启ComfyUI');
            return;
        }
        ...
        // 记录这个节点（不自动选中，避免移动视图）
        lastStickmanNode = node;
        
        // 显示创建提示消息
        showSuccessMessage('✅ 已成功添加火柴人编辑器导出图像节点到工作流');
        console.log('[hosepen] 火柴人编辑器 Image Output节点已添加');
        ...
        console.error('[hosepen] 创建火柴人编辑器节点失败:', error);
        throw error;
    }
}
